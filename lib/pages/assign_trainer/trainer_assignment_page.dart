...
///
///ASIGNAR UN SOLO ENTRENADOR
///

// Reemplaza todo el contenido de tu clase _TrainerAssignmentPageState con este:
/*
class _TrainerAssignmentPageState extends State<TrainerAssignmentPage> {
  final _formKey = GlobalKey<FormState>();
  ...
  final Set<int> _entrenadoresAsignados = {};
  ...
  Future<void> _asignarEntrenador() async {
    ...
    // Verifica si ya está asignado localmente
    ...
    if (success) {
      ...
      if (continuar == true) {
        setState(() {}); // Refresca los Dropdown
      } else {
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => const AssignmentSuccessPage()),
  ...
  if (success) {
    // Guarda el ID y nombre del entrenador
    _entrenadoresAsignados.add(trainerId);

    final nombreEntrenador = trainers.firstWhere(
      (trainer) => trainer['id'] == trainerId.toString(),
