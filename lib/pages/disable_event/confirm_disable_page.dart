...
class ConfirmDisablePage extends StatelessWidget {
  ...
  /// Deshabilita los eventos modificando su estado a "inactivo" en el backend.
  Future<void> _deshabilitarEventos(BuildContext context) async {
    ...
    try {
      for (var id in idsEventos) {
        final resultado = await repo.desactivarEventoRemoto(id);

        if (!resultado) {
          throw Exception("No se pudo desactivar el evento con id $id");
        }
      }
...
/*class ConfirmDisablePage extends StatelessWidget {
  ...
  /// Deshabilita los eventos modificando su estado a "inactivo".
  Future<void> _deshabilitarEventos(BuildContext context) async {
    final EventRepository repo = EventRepository();
